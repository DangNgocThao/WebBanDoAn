...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        if (empty($_FILES['file'])) {
            throw new Exception('Tải lên không hợp lệ!');
        }
...
<div class="col mt-3 mb-3">
    <h2 class="text-center">Thêm món mới</h2>
    <form method="post" enctype='multipart/form-data' class="w-50 m-auto">
        ...
        <div class="mb-3">
            <label for="file" class="form-label">Hình (<span class="text-danger">*</span>)</label>
            <input class="form-control" type="file" name="file" id="file" required />
            <span class='text-danger'><?= $fileErrors ?></span>
        </div>
        <div class="text-center">
            <button type="submit" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Thêm món</button>
